...
Ext.onReady(function() {
	...
	Ext.ux.newsCtrl = Ext.extend(Ext.Panel, {
		...
		initComponent : function() {
			this.items = [{
                ...
                listeners: {
                	'rowclick' : function rowclick(){
		            	var newsManage = Ext.getCmp('newsCtrl-grid');
			...
			Ext.ux.newsCtrl.superclass.initComponent.call(this);
		}
	});
	Ext.reg('newsCtrl', Ext.ux.newsCtrl);
	new Ext.Viewport({
	            		...
	            		var form = Ext.getCmp('addNews-form').getForm();
	                   	...
	                   	if (loadValid() && (alertMsg == "")) {
	                        ...
	                        Ext.Ajax.request({
									...
									params:{
			                        	'news.newsTitle': Ext.getCmp('newsTitle').getValue(),
										'news.newsAuthor': Ext.getCmp('newsAuth').getValue(),
										'newsSection': Ext.getCmp('section').getValue(),
			                        	'news.isPicNews': isPicNews,
		                            	'news.isNotice': isNotice,
		                            	'news.isAttach': isAttach,
		                            	'isEng':isEng,
		                            	'news.newsContent': KE.html('newsContent'),
		                            	'news.noticeValid':"2012-01-01GMT00:00:00",
		                            	'newsPicPath': '/upload/picnews/' + picPath,
		                            	'newsAttachPath': '/upload/attachment/' + attachPath,
			                        	'news.attachName':savePath_dt
			                        },
									success: function(form, action){
			                            Ext.Msg.alert('信息', '添加成功');
			                            newsWin.hide();
			                            Ext.getCmp('newsCtrl-grid').getStore().reload();
			                        },
	            ...
	            items: [{
	                ...
	                items: [{
	                    ...
	                    store: new Ext.data.JsonStore({
	                    	url: 'sectionAction!querySectByRole',
							root : 'sectionsList',
							baseParams: {
								roleId:parent.roleId
							},
...
/*	                    items: [{
	...
	function deleteNews() {
		Ext.Msg.confirm('信息', '确定删除新闻？' ,
			function(btn){
				if(btn == 'yes') {
					var newsGrid = Ext.getCmp('newsCtrl-grid');
					var sm = newsGrid.getSelectionModel();
					var store = Ext.getCmp('newsCtrl-grid').getStore();
			...
			newsWinU = new  Ext.Window({
					...
					handler: function() {
	            		...
	            		if (loadUpValid() && (alertMsg == "")) {
							...
							Ext.Ajax.request({
									...
									params:{
										'news.newsId': newsIdT,
			                        	'news.newsTitle': Ext.getCmp('newsTitleUp').getValue(),
										'newsSection': Ext.getCmp('newsSectionUp').getValue(),
		                            	'news.isNotice': isNotice,		                            
		                            	'news.newsContent': KE.html('newsContentUp'),
		                            	'news.noticeValid':"2012-01-01GMT00:00:00",
		        ...
		        items: [{
	                ...
	                items: [{
	                    ...
	                    blankText: '所属栏目不能为空',
	                    store: new Ext.data.JsonStore({
	                    	url: 'sectionAction!queryAllSections',
							root : 'sectionsList',
							autoLoad : true,
							fields: ['sectionName']
	                    }),
			...
			Ext.lib.Ajax.request(
				...
				'newsAction!queryNews',
				{
					success: function(response) {
						...
						newsIdT = data.news.newsId;
						var newsTitleT = data.news.newsTitle;
						var newsSectionT = data.news.section.sectionName;
						var newsAuthorT = data.news.newsAuthor;
						var isPicNewsT = data.news.isPicNews;
						var isNoticeT = data.news.isNotice;
						var isAttachT = data.news.isAttach;
						var newsContent = data.news.newsContent;
						var noticeDateT = data.news.noticeValid;
						var picPathT = data.news.picPath;
						attachPathT = data.news.attachPath;
