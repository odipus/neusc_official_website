/*******************************************************************************
 * KindEditor - WYSIWYG HTML Editor for Internet Copyright (C) 2006-2011 Longhao
 * Luo
 *
...
(function(undefined) {
	...
	KE.setting = {
		...
		items : ['source', '|', 'fullscreen', 'undo', 'redo', 'print', 'cut',
				...
				'title', 'fontname', 'fontsize', '|', 'textcolor', 'bgcolor',
				'bold', 'italic', 'underline', 'strikethrough', 'removeformat',
				'|', 'image', 'flash', 'media', 'advtable', 'hr',
				'emoticons', 'link', 'unlink', '|', 'format'],
		colorTable : [
		...
		inlineTags : ['b', 'del', 'em', 'font', 'i', 'span', 'strike',
				'strong', 'sub', 'sup', 'u'],
		endlineTags : ['br', 'hr', 'table', 'tbody', 'td', 'tr', 'th', 'div',
				'p', 'ol', 'ul', 'li', 'blockquote', 'h1', 'h2', 'h3', 'h4',
				'h5', 'h6', 'script', 'style', 'marquee'],
		htmlTags : {
			...
			table : ['border', 'cellspacing', 'cellpadding', 'width', 'height',
					...
					'.font-style', '.text-decoration', '.background', '.width',
					'.height'],
	...
	KE.event = {
		...
		remove : function(el, type, fn, id) {
			...
			if (id !== undefined) {
				...
				for (var i = 0, len = stack.length; i < len; i++) {
					var item = stack[i];
					if (item && el === item.el && type === item.type
							&& fn === item.fn) {
						delete stack[i];
					}
				}
		...
		ctrl : function(el, key, func, id) {
			key = key.toString().match(/^\d{2,}$/) ? key : key.toUpperCase()
					.charCodeAt(0);
			this.add(el, 'keydown', function(e) {
						if (e.ctrlKey && e.keyCode == key && !e.shiftKey
								&& !e.altKey) {
							func(e);
							KE.event.stop(e);
							return false;
	...
	KE.selection = function(doc) {
		...
		this.init = function() {
			...
			if (KE.browser.IE) {
				if (range.item) {
					...
					var getStartEnd = function(isStart) {
						...
						for (var i = 0, len = nodes.length; i < len; i++) {
							var node = nodes[i];
							var cmp = testRange.compareEndPoints(
									'StartToStart', pointRange);
							if (cmp > 0) {
								isEnd = true;
							} else if (cmp == 0) {
								if (node.nodeType == 1) {
							...
							if (node.nodeType == 1) {
								...
								// fix bug: typeof node.nodeValue can return
								// "unknown" in IE.
				...
				this.isControl = (startNode.nodeType == 1
						&& startNode === endNode && range.startOffset + 1 == range.endOffset);
				if (startNode.nodeType == 1 && endNode.nodeType == 3
						&& endPos == 0 && endNode.previousSibling) {
		...
		this.addRange = function(keRange) {
			...
			if (KE.browser.IE) {
				...
				if (!this.range.item) {
					...
					if (node == keRange.endNode
							&& KE.util.getNodeType(node) == 1
	...
	KE.range = function(doc) {
		...
		this.comparePoints = function(how, range) {
			var compareNodes = function(nodeA, posA, nodeB, posB) {
				...
				if (KE.browser.IE) {
					...
					if (rangeA.compareBoundaryPoints(Range.START_TO_START,
							rangeB) > 0) {
						cmp = 1;
					} else if (rangeA.compareBoundaryPoints(
							Range.START_TO_START, rangeB) == 0) {
						if (posA > posB)
							cmp = 1;
						else if (posA == posB)
							cmp = 0;
			...
			if (how == 'START_TO_START')
				return compareNodes(this.startNode, this.startPos,
						range.startNode, range.startPos);
			if (how == 'START_TO_END')
				return compareNodes(this.startNode, this.startPos,
						range.endNode, range.endPos);
			if (how == 'END_TO_START')
		...
		this.setTextStart = function(node, pos) {
			...
			KE.eachNode(node, function(n) {
						if (KE.util.getNodeType(n) == 3
								&& n.nodeValue.length > 0
								|| KE.util.getNodeType(n) == 88) {
							textNode = n;
							pos = 0;
							return false;
						}
		...
		this.selectTextNode = function(node) {
			...
			this.setTextEnd(node, node.nodeType == 1
							? 0
							: node.nodeValue.length);
		};
			...
			function extractNodes(parent, frag) {
				...
				while (node) {
					...
					if (type == 1) {
						...
						if (isStarted
								&& (cmp < 0 || (cmp == 0 && noEndTagHash[node.nodeName
	...
	KE.cmd = function(id) {
			...
			for (var i = 0, len = attr.length; i < len; i++) {
				KE.each(attr[i], function(key, value) {
							if (key.charAt(0) == '.') {
								...
								if (KE.browser.IE && KE.browser.VERSION < 8
										&& key == 'class')
									key = 'className';
		...
		this.wrapTextNode = function(node, startPos, endPos, element,
				attributes) {
			var length = node.nodeValue.length;
		...
		this.wrap = function(tagName, attributes) {
			...
			KE.eachNode(parentNode, function(node) {
				...
				if (node.nodeType == 1) {
					if (node == startNode && node == endNode) {
						if (KE.util.inArray(node.tagName.toLowerCase(),
								KE.g[id].noEndTags)) {
							if (startPos > 0)
					...
					if (isStarted) {
						if (node == startNode && node == endNode) {
							...
							var rangeNode = self.wrapTextNode(node, 0, endPos,
									element, attributes);
							keRange.setEnd(rangeNode, rangeNode.nodeType == 1
											? 0
											: rangeNode.nodeValue.length);
							return false;
						} else {
							self.wrapTextNode(node, 0, node.nodeValue.length,
									element, attributes);
						}
					}
				}
		...
		this.remove = function(tags) {
			...
			if (startParent) {
				var startFrags = this.splitNodeParent(startParent, startNode,
						startPos);
				keRange.setStart(startFrags.right, 0);
				if (startNode == endNode
						&& KE.util.getNodeTextLength(startFrags.right) > 0) {
	...
	KE.format = {
		...
		getHtml : function(html, htmlTags, urlType) {
			...
			html = html.replace(/(<pre[^>]*>)([\s\S]*?)(<\/pre>)/ig, function(
							$0, $1, $2, $3) {
						return $1 + $2.replace(/<br[^>]*>/ig, '\n') + $3;
					});
			var htmlTagHash = {};
			var fontSizeHash = ['xx-small', 'x-small', 'small', 'medium',
			...
			html = html.replace(re, function($0, $1, $2, $3, $4, $5, $6) {
				...
				if (isFilter && typeof htmlTagHash[tagName] == "undefined")
					return '';
				if (endSlash === ''
						&& typeof noEndTagHash[tagName] != "undefined")
					endSlash = ' /';
				...
				if (tagName === 'font') {
					...
					attr = attr.replace(
							...
							function($0, $1, $2) {
								...
								if (key === 'size') {
									style['font-size'] = fontSizeHash[parseInt(val)
											- 1]
											|| '';
									return ' ';
								}
				...
				if (attr !== '') {
					attr = attr.replace(
							...
							function($0, $1, $2) {
								...
								if (isFilter) {
									if (key.charAt(0) === "."
											|| (key !== "style" && typeof htmlTagHash[tagName][key] == "undefined"))
										return ' ';
								}
								if (val === '') {
									val = '""';
									...
									if (key === "style") {
										...
										val = val.replace(
												...
												function($0, $1, $2) {
													var k = $1.toLowerCase();
													if (isFilter) {
														if (typeof htmlTagHash[tagName]['style'] == "undefined"
																&& typeof htmlTagHash[tagName]['.'
																		+ k] == "undefined")
															return '';
													}
					...
					attr = attr.replace(
							...
							function($0, $1) {
								...
								if (isFilter) {
									if (key.charAt(0) === "."
											|| typeof htmlTagHash[tagName][key] == "undefined")
										return ' ';
								}
								return ' ' + key + '="' + key + '"' + ' ';
							});
					...
					if (attr)
						attr = ' ' + attr;
					return startNewline + '<' + startSlash + tagName + attr
							+ endSlash + '>' + endNewline;
				} else {
					return startNewline + '<' + startSlash + tagName + endSlash
							+ '>' + endNewline;
				}
			});
			if (!KE.browser.IE) {
			...
			var trimHtml = function(inHtml) {
				var outHtml = inHtml.replace(new RegExp('<(' + reg
										+ ')[^>]*><\\/(' + reg + ')>', 'ig'),
						function($0, $1, $2) {
							if ($1 == $2)
								return '';
	...
	KE.getComputedStyle = function(el, key) {
		var doc = el.ownerDocument, win = doc.parentWindow || doc.defaultView, jsKey = KE.util
				.getJsKey(key), val = '';
		if (win.getComputedStyle) {
			var style = win.getComputedStyle(el, null);
			val = style[jsKey] || style.getPropertyValue(key)
					|| el.style[jsKey];
	...
	KE.util = {
		...
		getJsKey : function(key) {
			...
			key = '';
			for (var i = 0, len = arr.length; i < len; i++) {
				key += (i > 0) ? arr[i].charAt(0).toUpperCase()
						+ arr[i].substr(1) : arr[i];
			}
			return key;
		},
		...
		setOpacity : function(el, opacity) {
			if (typeof el.style.opacity == "undefined") {
				el.style.filter = (opacity == 100) ? "" : "alpha(opacity="
						+ opacity + ")";
			} else {
		...
		getParam : function(url, name) {
			return url.match(new RegExp('[?&]' + name + '=([^?&]+)', 'i'))
					? unescape(RegExp.$1)
					: '';
		},
		createRange : function(doc) {
			return doc.body.createTextRange ? doc.body.createTextRange() : doc
					.createRange();
		},
		getNodeType : function(node) {
			return (node.nodeType == 1 && KE.util.inArray(node.tagName
							.toLowerCase(), KE.setting.noEndTags))
					? 88
					: node.nodeType;
		},
			...
			mousedownObj.onmousedown = function(e) {
				...
				function moveListener(e) {
					if (dragFlag) {
						var pos = KE.util.getCoords(e);
						var scrollPos = KE.util.getScrollPos();
						var top = parseInt(pos.y - mouseTop - scrollTop
								+ scrollPos.y);
						var left = parseInt(pos.x - mouseLeft - scrollLeft
								+ scrollPos.x);
				...
				function iframeMoveListener(e) {
					if (dragFlag) {
						...
						var top = parseInt(iframePos.y + pos.y - mouseTop
								- scrollTop);
						var left = parseInt(iframePos.x + pos.x - mouseLeft
								- scrollLeft);
						func(objTop, objLeft, objWidth, objHeight, top, left);
					}
				}
				...
				function upListener(e) {
					...
					KE.event.remove(g.iframeDoc, 'mousemove',
							iframeMoveListener);
		...
		resize : function(id, width, height, isCheck, isResizeWidth) {
			isResizeWidth = (typeof isResizeWidth == "undefined")
					? true
					: isResizeWidth;
			var g = KE.g[id];
			if (!g.container)
				return;
			if (isCheck
					&& (parseInt(width) <= g.minWidth || parseInt(height) <= g.minHeight))
				return;
			if (isResizeWidth)
			...
			if (diff >= 0) {
				...
				g.newTextarea.style.height = (((KE.browser.IE
						&& KE.browser.VERSION < 8 || document.compatMode != 'CSS1Compat') && diff >= 2)
						? diff - 2
						: diff)
						+ 'px';
			}
		...
		setDefaultPlugin : function(id) {
			var items = ['selectall', 'justifyleft', 'justifycenter',
			...
			for (var i = 0; i < items.length; i++) {
				...
				if (item in shortcuts) {
					plugin.init = (function(item) {
						return function(id) {
							KE.event.ctrl(KE.g[id].iframeDoc, shortcuts[item],
									function(e) {
		...
		getFullHtml : function(id) {
			...
			html += '<link href="' + KE.g[id].skinsPath
					+ 'common/editor.css?ver=' + escape(KE.version)
		...
		getMediaImage : function(id, type, attrs) {
			...
			var html = '<img class="' + className + '" src="'
					+ KE.g[id].skinsPath + 'common/blank.gif" ';
			if (style !== '')
				html += 'style="' + style + '" ';
		...
		getData : function(id, wyswygMode) {
			...
			if (!wyswygMode) {
				this.innerHtml(g.iframeDoc.body, KE.util.execSetHtmlHooks(id,
								g.newTextarea.value));
			}
		...
		setSelection : function(id) {
			...
			if (!KE.browser.IE
					|| keSel.range.item
					|| keSel.range.parentElement().ownerDocument === g.iframeDoc) {
				g.keSel = keSel;
		...
		innerHtml : function(el, html) {
			if (KE.browser.IE) {
				el.innerHTML = '<img id="__ke_temp_tag__" width="0" height="0" />'
						+ html;
		...
		insertHtml : function(id, html) {
			...
			if (KE.browser.IE) {
				...
				if (g.range.item) {
					...
					g.range
							.pasteHTML('<span id="__ke_temp_tag__">\u200B</span>'
									+ html);
					var node = KE.$('__ke_temp_tag__', g.iframeDoc);
					node.parentNode.removeChild(node);
				}
			...
			KE.event.add(g.iframeDoc, 'contextmenu', function(e) {
						...
						for (var i = 0, len = g.contextmenuItems.length; i < len; i++) {
							...
							if (item === '-') {
								...
								items.push(item);
								if (item.options) {
									var width = parseInt(item.options.width)
											|| 0;
									if (width > maxWidth)
										maxWidth = width;
								}
		...
		addNewlineEvent : function(id) {
			...
			KE.event.add(g.iframeDoc, 'keydown', function(e) {
						...
						if (g.newlineTag.toLowerCase() == 'br') {
							if (!KE.util.inArray(tagName, ['h1', 'h2', 'h3',
											'h4', 'h5', 'h6', 'li'])) {
								...
								if (KE.browser.IE) {
									...
									if (!nextNode) {
										...
										if (range.cloneContents().innerHTML
												.replace(
														/<(?!img|embed).*?>/ig,
														'') === '') {
											KE.util.pasteHtml(id, '<br />',
							...
							if (!KE.util.inArray(tagName, ['p', 'h1', 'h2',
											'h3', 'h4', 'h5', 'h6', 'pre',
											'div', 'li'])) {
	...
	KE.menu = function(arg) {
		...
		function init() {
			var width = arg.width;
			this.type = (arg.type && arg.type == 'contextmenu')
					? arg.type
					: 'menu';
			var div = KE.$$('div');
		...
		this.add = function(html, event, options) {
			...
			cDiv.onmouseover = function() {
				this.className = 'ke-' + self.type + '-item ke-' + self.type
						+ '-item-on';
				center.className = 'ke-' + self.type + '-center ke-'
						+ self.type + '-center-on';
			};
		...
		this.show = function() {
			...
			var pos = getPos.call(this, this.div.clientWidth,
	...
	KE.button = function(arg) {
		...
		span.className = 'ke-button-common ke-button-outer '
				+ (arg.className || '');
		span.title = arg.text;
		btn = KE.$$('input', doc);
	...
	KE.dialog = function(arg) {
		...
		function getPos() {
			...
			if (g.dialogAlignType == 'page') {
				...
				var el = g.container;
				var xDiff = Math.round(el.clientWidth / 2)
						- Math.round(width / 2);
				var yDiff = Math.round(el.clientHeight / 2)
						- Math.round(height / 2);
		...
		this.show = function() {
			...
			KE.util.drag(id, titleDiv, div, function(objTop, objLeft, objWidth,
							objHeight, top, left) {
						if (self.ondrag)
							self.ondrag(id);
						setLimitNumber();
			...
			var iframe = (KE.g[id].dialogStack.length == 0 && KE.g[id].dialog)
					? KE.g[id].dialog
					: KE.$$('iframe');
			if (arg.useFrameCSS) {
	...
	KE.toolbar = {
		updateState : function(id) {
			var cmdList = ['justifyleft', 'justifycenter', 'justifyright',
					'justifyfull', 'insertorderedlist', 'insertunorderedlist',
					'indent', 'outdent', 'subscript', 'superscript', 'bold',
		...
		create : function(id) {
			...
			for (var i = 0; i < length; i++) {
				...
				if (typeof defaultItemHash[cmd] == 'undefined') {
					...
					span.className = 'ke-common-icon ke-common-icon-url ke-icon-'
							+ cmd;
				}
				a.appendChild(span);
				cell.appendChild(a);
	...
	KE.history = {
		...
		undo : function(id) {
			...
			if (g.wyswygMode) {
				KE.util.innerHtml(g.iframeDoc.body, KE.util.execSetHtmlHooks(
								id, prevHtml));
			} else {
	...
	KE.focus = function(id, position) {
		...
		if (position === 'end') {
			KE.util.setSelection(id);
			if (!KE.g[id].sel)
				return; // issue #120: Sometimes Firefox does not get selection
	...
	KE.create = function(id, mode) {
		...
		var width = KE.g[id].width || srcTextarea.style.width
				|| srcTextarea.offsetWidth + 'px';
		var height = KE.g[id].height || srcTextarea.style.height
				|| srcTextarea.offsetHeight + 'px';
		var tableObj = KE.util.createTable();
		...
		KE.util.drag(id, bottomRight, container, function(objTop, objLeft,
						objWidth, objHeight, top, left) {
					if (KE.g[id].resizeMode == 2)
						KE.util.resize(id, (objWidth + left) + 'px',
								(objHeight + top) + 'px', true);
					else if (KE.g[id].resizeMode == 1)
						KE.util.resize(id, objWidth + 'px', (objHeight + top)
										+ 'px', true, false);
				});
		KE.util.drag(id, bottomLeft, container, function(objTop, objLeft,
						objWidth, objHeight, top, left) {
		...
		KE.g[id].getHtmlHooks.push(function(html) {
					...
					html = html.replace(/(<[^>]+\s)ke-(on\w+="[^"]+"[^>]*>)/ig,
	...
	KE.init = function(args) {
		...
		KE.each(KE.setting, function(key, val) {
					g[key] = (typeof args[key] == 'undefined')
							? val
							: args[key];
					g.config[key] = g[key];
				});
...
(function(KE, undefined) {
	...
	KE.plugin.cut = {
		click : function(id) {
			try {
				if (!KE.g[id].iframeDoc.queryCommandSupported('cut'))
					throw 'e';
			} catch (e) {
	...
	KE.plugin.copy = {
		click : function(id) {
			try {
				if (!KE.g[id].iframeDoc.queryCommandSupported('copy'))
					throw 'e';
			} catch (e) {
	...
	KE.plugin.paste = {
		click : function(id) {
			try {
				if (!KE.g[id].iframeDoc.queryCommandSupported('paste'))
					throw 'e';
			} catch (e) {
	...
	KE.plugin.fontname = {
		click : function(id) {
			...
			KE.each(fontName, function(key, value) {
				var html = '<span class="ke-reset" style="font-family: ' + key
						+ ';">' + value + '</span>';
				menu.add(html, function() {
							KE.plugin[cmd].exec(id, key);
						}, {
							checked : (font === key.toLowerCase() || font === value
									.toLowerCase())
						});
			});
			menu.show();
	...
	KE.plugin.fontsize = {
		click : function(id) {
			var fontSize = ['9px', '10px', '12px', '14px', '16px', '18px',
					'24px', '32px'];
			var cmd = 'fontsize';
			KE.util.selection(id);
			...
			for (var i = 0, len = fontSize.length; i < len; i++) {
				...
				var html = '<span class="ke-reset" style="font-size: ' + value
						+ ';">' + value + '</span>';
	...
	KE.plugin.title = {
		click : function(id) {
			...
			KE.each(title, function(key, value) {
				...
				var html = '<span class="ke-reset" style="' + style + '">'
						+ value + '</span>';
				menu.add(html, function() {
							KE.plugin[cmd].exec(id, '<' + key + '>');
						}, {
							height : (sizeHash[key] + 12) + 'px',
							checked : (block === key.toLowerCase() || block === value
									.toLowerCase())
						});
			});
			menu.show();
	...
	KE.plugin.emoticons = {
		click : function(id) {
			var self = this, cmd = 'emoticons', rows = 5, cols = 9, total = 135, startNum = 0, cells = rows
					* cols, pages = Math.ceil(total / cells), colsHalf = Math
					.floor(cols / 2), g = KE.g[id], path = g.pluginsPath
					+ 'emoticons/', allowPreview = (g.allowPreviewEmoticons === undefined)
					? true
					: g.allowPreviewEmoticons;
			KE.util.selection(id);
			...
			function createEmoticonsTable(pageNum) {
				...
				for (var i = 0; i < rows; i++) {
					...
					for (var j = 0; j < cols; j++) {
						...
						span.style.backgroundPosition = '-' + (24 * num)
								+ 'px 0px';
	...
	KE.plugin.flash = {
		init : function(id) {
			...
			KE.g[id].getHtmlHooks.push(function(html) {
						return html.replace(
								...
								function(imgStr) {
									var width = imgStr
											.match(/style="[^"]*;?\s*width:\s*(\d+)/i)
											? RegExp.$1
											: 0;
									var height = imgStr
											.match(/style="[^"]*;?\s*height:\s*(\d+)/i)
											? RegExp.$1
											: 0;
									width = width
											|| (imgStr
			...
			KE.g[id].setHtmlHooks.push(function(html) {
				return html
						.replace(
								...
								function($0) {
									var src = $0.match(/\s+src="([^"]+)"/i)
											? RegExp.$1
											: '';
									if ($0.match(/\s+kesrc="([^"]+)"/i))
										src = RegExp.$1;
									var width = $0.match(/\s+width="([^"]+)"/i)
											? RegExp.$1
											: 0;
									var height = $0
											.match(/\s+height="([^"]+)"/i)
									...
									attrs.width = width;
									attrs.height = height;
									return KE.util.getMediaImage(id, 'flash',
											attrs);
								});
			});
		},
		...
		exec : function(id) {
			...
			if (!this.check(id, url, width, height))
				return false;
				...
				var onloadFunc = function() {
					...
					if (typeof data === 'object' && 'error' in data) {
						if (data.error === 0) {
							...
							self
									.insert(id, url, title, width, height, 0,
											align);
						} else {
							alert(data.message);
							return false;
	...
	KE.plugin.link = {
		...
		exec : function(id) {
			...
			if (KE.browser.IE) {
				...
				isItem = (startNode.nodeType == 1 && startNode === endNode && startNode.nodeName
						.toLowerCase() != 'br');
			}
			var isEmpty = !isItem;
			if (!isItem)
				isEmpty = KE.browser.IE ? g.range.text === '' : g.range
						.toString() === '';
				...
				for (var i = 0, l = arr.length; i < l; i++) {
					if (arr[i].href.match(/\/?__ke_temp_url__$/)) {
						...
						if (target)
							...
							arr[i].removeAttribute('target');
					}
				}
				if (KE.browser.WEBKIT && isItem
						&& startNode.tagName.toLowerCase() == 'img') {
	...
	KE.plugin.media = {
						...
						return html
								.replace(
										...
										function($0, $1) {
											...
											attrs.width = width;
											attrs.height = height;
											return KE.util.getMediaImage(id,
													'', attrs);
										});
					});
		},
		...
		exec : function(id) {
			...
			if (!this.check(id, url, width, height))
				return false;
			var autostart = KE.$('autostart', dialogDoc).checked
					? 'true'
					: 'false';
	...
	KE.plugin.advtable = {
		...
		tablecellprop : function(id) {
			...
			var dialog = new KE.dialog({
				...
				yesClickFn : function(id) {
					var dialogDoc = KE.util.getIframeDoc(dialog.iframe), widthBox = KE
							.$('width', dialogDoc), heightBox = KE.$('height',
							dialogDoc), widthTypeBox = KE.$('widthType',
							dialogDoc), heightTypeBox = KE.$('heightType',
							dialogDoc), textAlignBox = KE.$('textAlign',
							dialogDoc), verticalAlignBox = KE.$(
							'verticalAlign', dialogDoc), borderBox = KE.$(
							'border', dialogDoc), borderColorBox = KE.$(
							'borderColor', dialogDoc), backgroundColorBox = KE
							.$('backgroundColor', dialogDoc), width = widthBox.value, height = heightBox.value, widthType = widthTypeBox.value, heightType = heightTypeBox.value, textAlign = textAlignBox.value, verticalAlign = verticalAlignBox.value, border = borderBox.value, borderColor = borderColorBox.innerHTML, backgroundColor = backgroundColorBox.innerHTML;
					if (!width.match(/^\d*$/)) {
						alert(KE.lang['invalidWidth']);
						widthBox.focus();
						return false;
		...
		tablecolinsert : function(id, offset) {
			var table = this.getSelectedTable(id), cell = this
					.getSelectedCell(id), index = cell.cellIndex + offset;
		...
		tablerowinsert : function(id, offset) {
			var table = this.getSelectedTable(id), row = this
					.getSelectedRow(id), newRow = table.insertRow(row.rowIndex
					+ offset);
		...
		tablecoldelete : function(id) {
			var table = this.getSelectedTable(id), cell = this
					.getSelectedCell(id), index = cell.cellIndex;
			for (var i = 0, len = table.rows.length; i < len; i++) {
		...
		init : function(id) {
			...
			var tableCmds = 'prop,cellprop,colinsertleft,colinsertright,rowinsertabove,rowinsertbelow,coldelete,rowdelete,insert,delete'
					.split(',');
			for (var i = 0, len = tableCmds.length; i < len; i++) {
				...
				KE.g[id].contextmenuItems.push({
					...
					options : {
						width : '170px',
						iconHtml : '<span class="ke-common-icon ke-common-icon-url ke-icon-'
								+ name + '"></span>'
					}
				});
			}
			KE.g[id].contextmenuItems.push('-');
			KE.g[id].setHtmlHooks.push(function(html) {
						return html.replace(/<table([^>]*)>/ig,
								function($0, $1) {
									if ($1.match(/\s+border=["']?(\d*)["']?/ig)) {
										var border = RegExp.$1;
										if ($1.indexOf(zeroborder) < 0
												&& (border === '' || border === '0')) {
		...
		exec : function(id) {
			var zeroborder = 'ke-zeroborder', dialogDoc = KE.util
					.getIframeDoc(this.dialog.iframe), modeBox = KE.$('mode',
					dialogDoc), rowsBox = KE.$('rows', dialogDoc), colsBox = KE
					.$('cols', dialogDoc), widthBox = KE.$('width', dialogDoc), heightBox = KE
					.$('height', dialogDoc), widthTypeBox = KE.$('widthType',
					dialogDoc), heightTypeBox = KE.$('heightType', dialogDoc), paddingBox = KE
					.$('padding', dialogDoc), spacingBox = KE.$('spacing',
					dialogDoc), alignBox = KE.$('align', dialogDoc), borderBox = KE
					.$('border', dialogDoc), borderColorBox = KE.$(
					'borderColor', dialogDoc), backgroundColorBox = KE.$(
					'backgroundColor', dialogDoc), rows = rowsBox.value, cols = colsBox.value, width = widthBox.value, height = heightBox.value, widthType = widthTypeBox.value, heightType = heightTypeBox.value, padding = paddingBox.value, spacing = spacingBox.value, align = alignBox.value, border = borderBox.value, borderColor = borderColorBox.innerHTML, backgroundColor = backgroundColorBox.innerHTML;
		...
		if (deleteArr.length > 0) {
			for (var i = deleteArr.length - 1; i >= 0; i--) {
				if (deleteArr[i][1] < insertIndex && !inserted) {// 要先插入，否则顺序不对
					html = html.substring(0, insertIndex + 1) + insertStr
							+ html.substring(insertIndex + 1, html.length);
					inserted = true;
				}
				html = html.substring(0, deleteArr[i][0])
						+ html.substring(deleteArr[i][1], html.length);
			}
		}
		if (!inserted) {
			html = html.substring(0, insertIndex + 1) + insertStr
